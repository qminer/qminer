...
namespace TQm {
	namespace TStorage {
		...
		private:
			...
			/// Helper function
			void SetCallback(v8::Handle<v8::Value>& CallbacksObj, v8::Persistent<v8::Function>& Callback, const TStr& Name);
			/// Sets all callbacks
			void InitCallbacks(v8::Handle<v8::Value>& CallbacksObj);
			/// Get field helper
			v8::Handle<v8::Value> GetField(const uint64& RecId, const int& FieldId) const;
		public:
			~TNodeJsFuncStore();
			TNodeJsFuncStore(const TWPt<TBase>& _Base, uint _StoreId, const TStr& _StoreNm, const TStoreSchema& StoreSchema, v8::Handle<v8::Value>& CallbacksObj);
