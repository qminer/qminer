    ...
    if (line[-1] != ')'):
        ...
        if "=" in line:
            ...
            if lhs not in globalVars:
                ...
                print("not OK!: globals should be unique and we are returning them!)")
                sys.exit()
            # prepare rhs
            rhs = temp[1].strip()
        else:
...
fw3.close()
        ...
        #print(obj + ":" + fun + ":" + str(isFun))
        #for sig in objDict[obj][fun]:
        #    print(obj + " : " + fun +  ((" : " + sig["funArgs"]) if sig["isFun"] else ""))
        ...
        #    print(sig)
        #else:
        #    print('fungola ' + key + ' return ' + sig["return"])
...
for line in fr:
    ...
    # if property: (len = vec.length)
    #  -- attach _ to rhs and object (if it is not global) 
    #     _vec.length = _len
    
    # if function: determine if it returns
    #   if not (vec.print()): return var = ""
    #   if it returns (num = vec.inner(vec2)): return var = _ + lhs
    #  attach _ to all input args and function (if it is not global)
    # write it down

# hashtable: function name : (inargs, outarg, summary)
