...
double TKDEModel::LeaveOneOutRateBandwidth(const TFltV& Hist_, const TFltV& Bandwidths, const TFltV& Thresholds_, const THistogramPMFModelType& KType, const double& Tol) {
    ...
    //PSOut FOut = TFOut::New("tesi.txt", true);
    //TFltV PMF; ComputeHistDensity(Hist, PMF, Bandwidths[MinIdx], KType);
    //for (int i = 0; i < HistLen; i++) {
    //    FOut->PutStr(PMF[i].GetStr());
    //    FOut->PutCh(' ');
    //}
    //for (int i = 0; i < HistLen; i++) {
    //    FOut->PutStr(Hist[i].GetStr());
    //    FOut->PutCh(' ');
    //}
    //FOut->PutLn();
    //printf("\n");
    //printf("\nbest bandwidth: %g, rule of thumb: %g, points:%d\n", Bandwidths[MinIdx].Val, RuleOfThumbHistBandwidth(Hist), (int)NumPoints);
