...
/////////////////////////////////////////////////
// Time-Stop-Watch
TTmStopWatch TTmStopWatch::GlobalStopWatch(true);

/////////////////////////////////////////////////
// Time-Window
void TTmWnd::Tick(const  uint64& TimeMSecs) {
	// first time it's special
	if (StartMSecs == 0) {
		// we assume now is the start
		StartMSecs = TimeMSecs;
	} else {
		// move to the next interval as long as we are out of interval
		while (true) {
			// record is behind already, stop
			if (StartMSecs > TimeMSecs) { break; }
			// not enough time has passed to start a new time window
			if ((TimeMSecs - StartMSecs) < TimeWndMSecs) { break; }
			// it is, do the move
			if (Callback != NULL) { Callback->NewTimeWnd(TimeWndMSecs, StartMSecs); }
			// increase start of the current time window
			StartMSecs += TimeWndMSecs;
		}
	}
}

void TTmWnd::GetTimeWnd(TTm& StartTm, TTm& EndTm) const {
	StartTm = TTm::GetTmFromMSecs(GetTimeWndStart());
	EndTm = TTm::GetTmFromMSecs(GetTimeWndStart() + GetTimeWndSize());
}
