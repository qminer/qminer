...
TBagOfWords::TBagOfWords(const bool& TfP, const bool& IdfP, const bool& NormalizeP, 
        PTokenizer _Tokenizer, const int& _HashDim, const bool& _StoreHashWordsP,
    ...
    if (_HashDim != -1) { 
        ...
        StoreHashWordsP = _StoreHashWordsP;
        if (StoreHashWordsP) { HashWordH.Gen(HashDim); }
...
TBagOfWords::TBagOfWords(TSIn& SIn): Type(SIn),
    Tokenizer(TTokenizer::Load(SIn)), TokenSet(SIn), HashDim(SIn), StoreHashWordsP(SIn),
    NStart(SIn),NEnd(SIn), Docs(SIn), DocFqV(SIn), ForgetP(SIn), OldDocs(SIn),
    OldDocFqV(SIn), HashWordH(SIn) { }
...
bool TBagOfWords::Update(const TStrV& TokenStrV) {    
    ...
    if (IsHashing()) {  
        ...
        for (int TokenStrN = 0; TokenStrN < NgramStrV.Len(); TokenStrN++) {
            ...
            if (StoreHashWordsP) { HashWordH[TokenId].AddKey(TokenStr); }
        }
