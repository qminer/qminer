...
class TSvmParam{
  public:
    ...
    TSvmParam() : Type(C_SVC), Kernel(LINEAR), Degree(0), Gamma(1.0), Coef0(1.0) {  } //fill with default values
    TSvmParam(TInt _Type, TInt _Kernel, TInt _Degree, TFlt _Gamma, TFlt _Coef0) : 
...
private:
    ...
    TInt NSupportVectors; /// number of support vectors*/
    TFltVV SupportVectors; /// support vectors
    TFltVV Coef; /// coefficients for support vectors
    TFltV Rho; /// bias terms
public:
    TLinModel(): Bias(0.0) { Param = TSvmParam(); }
    TLinModel(const TFltV& _WgtV): WgtV(_WgtV), Bias(0.0) { Param = TSvmParam(); }
    TLinModel(const TFltV& _WgtV, const double& _Bias): WgtV(_WgtV), Bias(_Bias) { Param = TSvmParam(); }
    TLinModel(TInt _Type, TInt _Kernel, TInt _Degree, TFlt _Gamma, TFlt _Coef0): Bias(0.0) { Param = TSvmParam(_Type, _Kernel, _Degree, _Gamma, _Coef0); }
    ...
    svm_model_t* GetModelStruct() const {
          ...
          //svm_model->nr_class = NClass;
          //svm_model->l = NSupportVectors;
    ...
    void LibSvmFit(const TVec<TIntFltKdV>& VecV, const TFltV& TargetV, double Cost, double Unbalance, double Nu, double Eps, double CacheSize, double P,
        ...
        /*NClass = svm_model->nr_class;
        NSupportVectors = svm_model->l;*/
